...
def compare_results(old_results, new_results):
    ...
    print(f"Different outputs: {differences}/{len(TEST_CASES)} ({differences/len(TEST_CASES)*100:.1f}%)")
    print(f"Identical outputs: {len(TEST_CASES)-differences}/{len(TEST_CASES)} ({(len(TEST_CASES)-differences)/len(TEST_CASES)*100:.1f}%)")
    ...
    print(f"  Difference: {abs(new_avg - old_avg):.2f}s ({((new_avg - old_avg) / old_avg * 100):+.1f}%)")
    print(f"{'='*80}\n")
...
def main():
    ...
    old_results = test_with_parameters(
        ...
        top_k=40,
        min_p=0.01,
        label="OLD PARAMETERS (Deterministic)"
    )
