...
def test_grmr_v3_cpu():
    ...
    test_cases = [
        {
            "input": "There going to the store their.",
            "expected_fixes": ["They're", "there"]
        },
        {
        ...
        {
            "input": "Me and him went to school together.",
            "expected_fixes": ["He and I"]
        },
        {
            "input": "She should of went to the party.",
            "expected_fixes": ["have gone"]
        }
    ]
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        if corrected != input_text and has_fixes:
            print(f"  Status:   ✅ PASSED (corrections applied)")
            passed += 1
        elif corrected == input_text:
            print(f"  Status:   ⚠️  WARNING (no changes made)")
            failed += 1
        else:
            print(f"  Status:   ✅ PASSED (text modified)")
            passed += 1
