...
class PipelineRunner:
    ...
    def _get_filters(self):
        ...
        if file_extension.lower() == ".md":
            ...
            raise ValueError(
                f"Unsupported file type '{file_extension}'. Please provide a .md or .epub file."
            )
        ...
        filters.extend([(TTSNormalizer(), False), (output_generator, False)])
        ...
        for f, returns_stats in self.filters:
            ...
            try:
                original_text_blocks = (
                    [block["content"] for block in data.get("text_blocks", [])]
                ...
                if "text_blocks" in data:
                    changes = sum(
                        ...
                        if i < len(original_text_blocks)
                        and block["content"] != original_text_blocks[i]
                    )
                    log_extra["changes"] = changes

                self.logger.info(
                    f"Filter {f.__class__.__name__} executed successfully.",
                ...
                log_extra["duration_ms"] = int((end_time - start_time) * 1000)
                self.logger.error(
                    f"Filter {f.__class__.__name__} failed.", extra={"extra_data": log_extra}
                )
                if self.fail_fast:
                    raise
        return data
...
Note: Cannot use --use-t5 and --use-grmr simultaneously.
    ...
    parser.add_argument(
        "--fail-fast", action="store_true", help="Stop pipeline on first filter failure."
    )
    parser.add_argument(
        "--use-t5",
        action="store_true",
    ...
    try:
        ...
        if result and "output_filepath" in result:
            logging.info(
                f"File processed successfully. Corrected content written to: {result['output_filepath']}"
            )
        else:
