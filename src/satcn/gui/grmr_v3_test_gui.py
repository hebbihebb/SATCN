...
class GRMRV3TestGUI:
    ...
    def _create_widgets(self):
        ...
        device_combo = ttk.Combobox(
            gpu_frame, textvariable=self.device_var, values=["cuda", "cpu", "auto"], 
        ...
        temp_spin = ttk.Spinbox(
            gpu_frame, textvariable=self.temp_var, from_=0.1, to=1.0, 
            increment=0.1, width=10
        )
        temp_spin.grid(row=0, column=3, sticky="w")
        
        ttk.Label(gpu_frame, text="Top-P:").grid(row=1, column=0, sticky="w", padx=(0, 10), pady=(5, 0))
        self.top_p_var = tk.StringVar(value="0.15")
        top_p_spin = ttk.Spinbox(
            gpu_frame, textvariable=self.top_p_var, from_=0.01, to=1.0, 
            increment=0.05, width=10
        )
        top_p_spin.grid(row=1, column=1, sticky="w", pady=(5, 0))
        
        ttk.Label(gpu_frame, text="Max Tokens:").grid(row=1, column=2, sticky="w", padx=(20, 10), pady=(5, 0))
        self.max_tokens_var = tk.StringVar(value="256")
        max_tokens_spin = ttk.Spinbox(
            gpu_frame, textvariable=self.max_tokens_var, from_=128, to=2048, 
            increment=128, width=10
        )
    ...
    def browse_file(self):
        ...
        if path.suffix.lower() not in SUPPORTED_EXTENSIONS:
            messagebox.showerror(
                "Unsupported file", "Please choose a .txt or .md file."
            )
            return
    ...
    def run_correction(self):
        ...
        try:
            ...
            messagebox.showerror("Invalid parameters", "Please check temperature, top-p, and max tokens values.")
            self._reset_ui()
            return
    ...
    def _process_correction(self, text: str, device: str, temperature: float, top_p: float, max_tokens: int):
        """Process correction in background thread."""
        try:
            # Update status
            self.root.after(0, lambda: self.progress_var.set("Initializing GRMR-V3 model..."))
            self.root.after(0, lambda: self.status_var.set("Loading model with GPU acceleration..."))
            ...
            self.root.after(0, lambda: self.status_var.set("Running grammar correction with GPU..."))
            ...
            # Update status
            status_msg = f"✓ Complete! Processed {word_count:,} words in {elapsed:.1f}s ({wpm:,} wpm)"
