...
TEST_CORPUS = [
    ...
    TestCase(
        "The dogs was barking loudly.", ["subject-verb agreement"], category="grammar:sv-agreement"
    ),
    TestCase(
        "He don't like vegetables.", ["subject-verb agreement"], category="grammar:sv-agreement"
    ),
    # Grammar: Tense consistency
    TestCase("Yesterday I go to the market.", ["tense"], category="grammar:tense"),
    TestCase("She will went to the party tomorrow.", ["tense"], category="grammar:tense"),
    # Grammar: Common errors
    TestCase("Their going too fast.", ["they're vs their"], category="grammar:homophones"),
    TestCase("Its a beautiful day.", ["it's vs its"], category="grammar:homophones"),
    TestCase("Your going to love this.", ["you're vs your"], category="grammar:homophones"),
    # Spelling errors
    TestCase("Thiss sentnce have two speling errrors.", ["spelling"], category="spelling"),
    TestCase("I recieved the package yesteday.", ["spelling"], category="spelling"),
    TestCase("The resturant has excellant food.", ["spelling"], category="spelling"),
    ...
    TestCase("I could of done better.", ["could have"], category="grammar:common-errors"),
    TestCase("She should of known better.", ["should have"], category="grammar:common-errors"),
    ...
    # Punctuation
    TestCase("Where are you going", ["missing punctuation"], category="punctuation"),
    TestCase("Hello how are you doing today", ["missing punctuation"], category="punctuation"),
    # Contextual corrections
    TestCase(
        "The woman which I met was nice.", ["who vs which"], category="grammar:relative-pronouns"
    ),
    TestCase(
        "The car who I bought is red.", ["which vs who"], category="grammar:relative-pronouns"
    ),
    # Articles
    TestCase("I saw a elephant at the zoo.", ["a vs an"], category="grammar:articles"),
    TestCase(
...
def analyze_correction(
    original: str, corrected: str, expected_corrections: list[str], preserve_elements: list[str]
) -> dict:
    """
...
def run_benchmark(verbose: bool = True) -> dict:
    ...
    for i, test_case in enumerate(TEST_CORPUS, 1):
        ...
        if verbose:
            ...
            if analysis["changed"]:
                print("  ✓ Changed" if analysis["likely_correct"] else "  ⚠️  Changed (unexpected)")
            else:
                print(
                    "  ✓ Unchanged"
                    if analysis["likely_correct"]
                    else "  ⚠️  Unchanged (should have corrected)"
                )
            if not analysis["preserved_elements"]:
                print("  ✗ FAILED TO PRESERVE ELEMENTS")
...
def print_summary(summary: dict):
    ...
    print(
        f"  Correct behavior: {summary['correct_count']}/{summary['total_tests']} ({summary['accuracy']*100:.1f}%)"
    )
...
def save_report(summary: dict, output_path: str):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        for i, result in enumerate(summary["results"], 1):
            ...
            f.write(
                f"**Preserved elements:** {'Yes' if analysis['preserved_elements'] else 'No'}\n\n"
            )
            if analysis["notes"]:
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Run comprehensive quality benchmark for GRMR-V3")
    parser.add_argument(
        "--output", "-o", help="Save detailed report to markdown file", default=None
    )
